...
namespace NinjaTrader.Custom.Indicators.Customs.DrawingTools
{
    /// <summary>
    /// Represents an interface that exposes information regarding a Risk Reward IDrawingTool.
    /// </summary>
    // if you want ro replace the NT app RiskReward, please reverse these modified lines
    //RiskReward in Currency: modified 
    //	public class RiskReward : DrawingTool        //please uncomment for replacing the native NT RiskReward app
    //to
    public class RiskRewardAB3 : DrawingTool     //please comment out replacing the native NT RiskReward app
                                                 //RiskReward in Currency up
    {
        ...
        //RiskReward in Currency
        private int instrumentquantity = 1;
        private bool needsInstrumentquantityUpdate = true;
        private double dollarcentpertick = 1;
        private bool needsdollarcentpertickUpdate = true;


        private double accountvalue = 100000;
        private double percentriskpertrade = 1;
        private double riskpertrade = 1000;
        //RiskReward in Currency up
        ...
        [NinjaScriptProperty]
        [Display(ResourceType = typeof(Custom.Resource), Name = "Instrument Quantity", GroupName = "NinjaScriptGeneral", Order = 5)]
        public int Instrumentquantity
        {
            get { return instrumentquantity; }
            set
            {
                if (instrumentquantity < 1)
                    return;
                instrumentquantity = value;
                needsInstrumentquantityUpdate = true;
            }
        }

        [Range(0, double.MaxValue)]
        [NinjaScriptProperty]
        [Display(ResourceType = typeof(Custom.Resource), Name = "Dollarcents per tick", GroupName = "NinjaScriptGeneral", Order = 6)]
        public double Dollarcentpertick
        {
            get { return dollarcentpertick; }
            set
            {
                if (dollarcentpertick < 0)
                    return;
                dollarcentpertick = value;
                needsdollarcentpertickUpdate = true;
            }
        }

        [Range(0, double.MaxValue)]
        [NinjaScriptProperty]
        [Display(ResourceType = typeof(Custom.Resource), Name = "Account Value", GroupName = "NinjaScriptGeneral", Order = 6)]
        ...
        [Range(0, double.MaxValue)]
        [NinjaScriptProperty]
        [Display(ResourceType = typeof(Custom.Resource), Name = "Percent Risk of Account Value per trade", GroupName = "NinjaScriptGeneral", Order = 6)]
        public double Percentriskpertrade
        {
            get { return percentriskpertrade; }
            set
            {
                if (percentriskpertrade < 0)
                    return;
                percentriskpertrade = value;
                //		needsdollarcentpertickUpdate	= true; ??                                                                                    
            }
        }
        ...
        [Range(0, double.MaxValue)]
        [NinjaScriptProperty]
        [Display(ResourceType = typeof(Custom.Resource), Name = "$ risk per trade", GroupName = "NinjaScriptGeneral", Order = 6)]
        ...
        //RiskReward in Currency
        [Display(ResourceType = typeof(Custom.Resource), Name = "Show Risk/Target in Ticks (x) or Currency ( )?", GroupName = "NinjaScriptGeneral", Order = 4)]
        public bool tickordollarcent { get; set; }

        [Display(ResourceType = typeof(Custom.Resource), Name = "Show Maximum Instrument Quantity: yes (x), no ( )?", GroupName = "NinjaScriptGeneral", Order = 4)]
        public bool maxsharenumber { get; set; }

        [Display(ResourceType = typeof(Custom.Resource), Name = "Risk Based on: %[account] (x) or $ ( )?", GroupName = "NinjaScriptGeneral", Order = 4)]
        public bool riskpertradeinpercentaccount { get; set; }
        //RiskReward in Currency up
        ...
        public override bool IsAlertConditionTrue(AlertConditionItem conditionItem, NinjaScript.Condition condition, ChartAlertValue[] values, ChartControl chartControl, ChartScale chartScale)
        {
            ...
            switch (condition)
            {
                case NinjaScript.Condition.Greater: return pointLocation == MathHelper.PointLineLocation.LeftOrAbove;
                case NinjaScript.Condition.GreaterEqual: return pointLocation == MathHelper.PointLineLocation.LeftOrAbove || pointLocation == MathHelper.PointLineLocation.DirectlyOnLine;
                case NinjaScript.Condition.Less: return pointLocation == MathHelper.PointLineLocation.RightOrBelow;
                case NinjaScript.Condition.LessEqual: return pointLocation == MathHelper.PointLineLocation.RightOrBelow || pointLocation == MathHelper.PointLineLocation.DirectlyOnLine;
                case NinjaScript.Condition.Equals: return pointLocation == MathHelper.PointLineLocation.DirectlyOnLine;
                case NinjaScript.Condition.NotEqual: return pointLocation != MathHelper.PointLineLocation.DirectlyOnLine;
                case NinjaScript.Condition.CrossAbove:
                case NinjaScript.Condition.CrossBelow:
                    Predicate<ChartAlertValue> predicate = v =>
                    {
                        ...
                        if (condition == NinjaScript.Condition.CrossAbove)
        ...
        public override void OnRender(ChartControl chartControl, ChartScale chartScale)
        {
            ...
            if (DrawTarget)
            {
                ...
                //RiskReward in Currency
                Point betweenTargetandEntry = new Point(0, 0);
                betweenTargetandEntry.X = (targetPoint.X + entryPoint.X) / 2;
                betweenTargetandEntry.Y = (targetPoint.Y + entryPoint.Y) / 2;
                double betweenTargetandEntryPrice = Math.Abs(targetPrice - entryPrice);
                double quantitybydollarcentpertick = dollarcentpertick * instrumentquantity;
                //RiskReward in Currency up

                DrawPriceText(RewardAnchor, targetPoint, targetPrice, chartControl, chartPanel, chartScale);

                //RiskReward in Currency
                if (maxsharenumber == false)
                {
                    if (tickordollarcent == true)
                    {
                        DrawPriceText(RewardAnchor, betweenTargetandEntry, betweenTargetandEntryPrice, chartControl, chartPanel, chartScale);
                    }
                    else
                    { DrawPriceText(RewardAnchor, betweenTargetandEntry, quantitybydollarcentpertick * betweenTargetandEntryPrice, chartControl, chartPanel, chartScale); }
                }
                else
                {
                }
                ...
                double betweenStoplossandEntryPrice = Math.Abs(stopPrice - entryPrice);
                double quantitybydollarcentpertick2 = dollarcentpertick * instrumentquantity;
                //RiskReward in Currency up
                ...
                if (maxsharenumber == false)
                {
                    if (tickordollarcent == true)
                    {
                        DrawPriceText(RiskAnchor, betweenStoplossandEntry, betweenStoplossandEntryPrice, chartControl, chartPanel, chartScale);
                    }
                    else
                    { DrawPriceText(RiskAnchor, betweenStoplossandEntry, quantitybydollarcentpertick2 * betweenStoplossandEntryPrice, chartControl, chartPanel, chartScale); }
                }
                else
                {
                    if (riskpertradeinpercentaccount == true)
                    //if from percentrisk pertrade     or  from absolute risk per trade
                    {
                        DrawPriceText(RiskAnchor, betweenStoplossandEntry, Math.Round(((accountvalue * percentriskpertrade / 100) / betweenStoplossandEntryPrice) / dollarcentpertick, 0), chartControl, chartPanel, chartScale);
                    }
                    else
                    {
                        DrawPriceText(RiskAnchor, betweenStoplossandEntry, (riskpertrade / betweenStoplossandEntryPrice) / dollarcentpertick, chartControl, chartPanel, chartScale);
                    }
                }

                //		private double			accountvalue =0;
                //		private double			percentriskpertrade =0;
                //      private double			riskpertrade =1;
                //RiskReward in Currency up		
        ...
        protected override void OnStateChange()
        {
            if (State == State.SetDefaults)
            {
                ...
                // if you want ro replace the NT app RiskReward, please reverse these modified lines
                //RiskReward in Currency: modified
                //              Name						= Custom.Resource.NinjaScriptDrawingToolRiskRewardName;  //please uncomment for replacing the native NT RiskReward app
                //to
                Name = "RiskRewardAB3";                                       //please comment out for replacing the native NT RiskReward app
                                                                              //RiskReward in Currency up
                Ratio = 2;
                ...
                Dollarcentpertick = 1;
                tickordollarcent = true;
                maxsharenumber = false;
                riskpertradeinpercentaccount = true;
                //RiskReward in Currency up
        ...
        public void SetReward()
        {
            ...
            //RiskReward in Currency
            needsInstrumentquantityUpdate = false;
            needsdollarcentpertickUpdate = false;
            //RiskReward in Currency  up
        }
    ...
    public static partial class Draw
    {
        ...
        //to
        //please uncomment for replacing the native NT RiskReward app		
        //		private static RiskReward RiskRewardCore(NinjaScriptBase owner, string tag,
        //please comment out for replacing the native NT RiskReward app		
        private static RiskReward RiskRewardCoreAB3(NinjaScriptBase owner, string tag,
        ...
        //RiskReward in Currency
        /// <param name="instrumentquantity">An int value determining the risk dependent on the quantity</param>
        /// <param name="dollarcentpertick>An double value determining the dollarcent value of a tick</param>
        /// <param name="tickordollarcent>Should target and risk be displayed in ticks or dollarcent amounts</param>
        //RiskReward in Currency up		
        ...
        //please uncomment for replacing the native NT RiskReward app	
        //		public static RiskReward RiskReward(NinjaScriptBase owner, string tag, bool isAutoScale, DateTime entryTime, double entryY, DateTime endTime, double endY, double ratio, int instrumentquantity, double dollarcentpertick, bool	tickordollarcent, bool isStop)
        //    	{
        //			return isStop
        //				? RiskRewardCore(owner, tag, isAutoScale, int.MinValue, entryTime, entryY, int.MinValue, endTime, endY, 0, Core.Globals.MinDate, 0, ratio, instrumentquantity, dollarcentpertick, tickordollarcent, true, false, null)
        //				: RiskRewardCore(owner, tag, isAutoScale, int.MinValue, entryTime, entryY, 0, Core.Globals.MinDate, 0, int.MinValue, endTime, endY, ratio, instrumentquantity, dollarcentpertick, tickordollarcent, false, false, null);
        //		}

        //please comment out for replacing the native NT RiskReward app
        public static RiskReward RiskRewardAB3(NinjaScriptBase owner, string tag, bool isAutoScale, DateTime entryTime, double entryY, DateTime endTime, double endY, double ratio, int instrumentquantity, double dollarcentpertick, bool tickordollarcent, bool isStop)
        {
            return isStop
                ? RiskRewardCoreAB3(owner, tag, isAutoScale, int.MinValue, entryTime, entryY, int.MinValue, endTime, endY, 0, Core.Globals.MinDate, 0, ratio, instrumentquantity, dollarcentpertick, tickordollarcent, true, false, null)
                : RiskRewardCoreAB3(owner, tag, isAutoScale, int.MinValue, entryTime, entryY, 0, Core.Globals.MinDate, 0, int.MinValue, endTime, endY, ratio, instrumentquantity, dollarcentpertick, tickordollarcent, false, false, null);
        }
        ...
        // to
        //please uncomment for replacing the native NT RiskReward app
        //		public static RiskReward RiskReward(NinjaScriptBase owner, string tag, bool isAutoScale, int entryBarsAgo, double entryY, int endBarsAgo, double endY, double ratio, int instrumentquantity, double dollarcentpertick, bool	tickordollarcent, bool isStop)
        //		{
        //			return isStop
        //				? RiskRewardCore(owner, tag, isAutoScale, entryBarsAgo, Core.Globals.MinDate, entryY, endBarsAgo, Core.Globals.MinDate, endY, 0, Core.Globals.MinDate, 0, ratio, instrumentquantity, dollarcentpertick, tickordollarcent, true, false, null)
        //				: RiskRewardCore(owner, tag, isAutoScale, entryBarsAgo, Core.Globals.MinDate, entryY, 0, Core.Globals.MinDate, 0, endBarsAgo, Core.Globals.MinDate, endY, ratio, instrumentquantity, dollarcentpertick, tickordollarcent, false, false, null);
        //		}

        //please comment out for replacing the native NT RiskReward app
        public static RiskReward RiskRewardAB3(NinjaScriptBase owner, string tag, bool isAutoScale, int entryBarsAgo, double entryY, int endBarsAgo, double endY, double ratio, int instrumentquantity, double dollarcentpertick, bool tickordollarcent, bool isStop)
        {
            return isStop
                ? RiskRewardCoreAB3(owner, tag, isAutoScale, entryBarsAgo, Core.Globals.MinDate, entryY, endBarsAgo, Core.Globals.MinDate, endY, 0, Core.Globals.MinDate, 0, ratio, instrumentquantity, dollarcentpertick, tickordollarcent, true, false, null)
                : RiskRewardCoreAB3(owner, tag, isAutoScale, entryBarsAgo, Core.Globals.MinDate, entryY, 0, Core.Globals.MinDate, 0, endBarsAgo, Core.Globals.MinDate, endY, ratio, instrumentquantity, dollarcentpertick, tickordollarcent, false, false, null);
        }
        //RiskReward in Currency
        ...
        //to
        //please uncomment for replacing the native NT RiskReward app
        //		public static RiskReward RiskReward(NinjaScriptBase owner, string tag, bool isAutoScale, DateTime entryTime, double entryY, DateTime endTime, double endY, double ratio, int instrumentquantity, double dollarcentpertick, bool	tickordollarcent, bool isStop, bool isGlobal, string templateName)
        //		{
        //			return isStop
        //				? RiskRewardCore(owner, tag, isAutoScale, int.MinValue, entryTime, entryY, int.MinValue, endTime, endY, 0, Core.Globals.MinDate, 0, ratio, instrumentquantity, dollarcentpertick, tickordollarcent, true, isGlobal, templateName)
        //				: RiskRewardCore(owner, tag, isAutoScale, int.MinValue, entryTime, entryY, 0, Core.Globals.MinDate, 0, int.MinValue, endTime, endY, ratio, instrumentquantity, dollarcentpertick, tickordollarcent, false, isGlobal, templateName);
        //		}
        //please comment out for replacing the native NT RiskReward app
        public static RiskReward RiskRewardAB3(NinjaScriptBase owner, string tag, bool isAutoScale, DateTime entryTime, double entryY, DateTime endTime, double endY, double ratio, int instrumentquantity, double dollarcentpertick, bool tickordollarcent, bool isStop, bool isGlobal, string templateName)
        {
            return isStop
                ? RiskRewardCoreAB3(owner, tag, isAutoScale, int.MinValue, entryTime, entryY, int.MinValue, endTime, endY, 0, Core.Globals.MinDate, 0, ratio, instrumentquantity, dollarcentpertick, tickordollarcent, true, isGlobal, templateName)
                : RiskRewardCoreAB3(owner, tag, isAutoScale, int.MinValue, entryTime, entryY, 0, Core.Globals.MinDate, 0, int.MinValue, endTime, endY, ratio, instrumentquantity, dollarcentpertick, tickordollarcent, false, isGlobal, templateName);
        }
        //RiskReward in Currency up	
        ...
        //to		
        //please uncomment for replacing the native NT RiskReward app	
        //		public static RiskReward RiskReward(NinjaScriptBase owner, string tag, bool isAutoScale, int entryBarsAgo, double entryY, int endBarsAgo, double endY, double ratio, int instrumentquantity, double dollarcentpertick, bool	tickordollarcent, bool isStop, bool isGlobal, string templateName)
        //		{
        //			return isStop
        //				? RiskRewardCore(owner, tag, isAutoScale, entryBarsAgo, Core.Globals.MinDate, entryY, endBarsAgo, Core.Globals.MinDate, endY, 0, Core.Globals.MinDate, 0, ratio, instrumentquantity, dollarcentpertick, tickordollarcent, true, isGlobal, templateName)
        //				: RiskRewardCore(owner, tag, isAutoScale, entryBarsAgo, Core.Globals.MinDate, entryY, 0, Core.Globals.MinDate, 0, endBarsAgo, Core.Globals.MinDate, endY, ratio, instrumentquantity, dollarcentpertick, tickordollarcent, false, isGlobal, templateName);
        //		}
        //please comment out for replacing the native NT RiskReward app	
        public static RiskReward RiskRewardAB3(NinjaScriptBase owner, string tag, bool isAutoScale, int entryBarsAgo, double entryY, int endBarsAgo, double endY, double ratio, int instrumentquantity, double dollarcentpertick, bool tickordollarcent, bool isStop, bool isGlobal, string templateName)
        {
            return isStop
                ? RiskRewardCoreAB3(owner, tag, isAutoScale, entryBarsAgo, Core.Globals.MinDate, entryY, endBarsAgo, Core.Globals.MinDate, endY, 0, Core.Globals.MinDate, 0, ratio, instrumentquantity, dollarcentpertick, tickordollarcent, true, isGlobal, templateName)
                : RiskRewardCoreAB3(owner, tag, isAutoScale, entryBarsAgo, Core.Globals.MinDate, entryY, 0, Core.Globals.MinDate, 0, endBarsAgo, Core.Globals.MinDate, endY, ratio, instrumentquantity, dollarcentpertick, tickordollarcent, false, isGlobal, templateName);
        }
